...
TEST(elas_fs_iso_segment,U_0_1){
    ...
    double eps = 1.e-6;
    std::cout << Ue(0,0) << "\n";
    ASSERT_TRUE(abs(Ue(0,0)-U_exact(0,0))<eps && abs(Ue(1,0)-U_exact(1,0))<eps&& abs(Ue(1,1)-U_exact(1,1))<eps && abs(Ue(0,1)-U_exact(0,1))<eps);
}
...
TEST(elas_fs_iso_segment,U_0_3){
   ...
   // {{0.00202768, 0.000574292}, {0.000574292, 0.0044795}}
    U_exact(0,0)=0.00202768;U_exact(0,1)=0.000574292;
    U_exact(1,0)=0.000574292;U_exact(1,1)=0.0044795;
...
TEST(elas_fs_iso_segment,U_0_4){
    ...
    U_exact(0,0)=0.0091904;U_exact(0,1)=-0.00109817;
    U_exact(1,0)=-0.00109817;U_exact(1,1)=0.00593223;
    double eps = 1.e-6;
    ASSERT_TRUE(abs(Ue(0,0)-U_exact(0,0))<eps && abs(Ue(1,0)-U_exact(1,0))<eps&& abs(Ue(1,1)-U_exact(1,1))<eps && abs(Ue(0,1)-U_exact(0,1))<eps);
}
...
TEST(elas_fs_iso_segment,S_0_1){
  ...
  //  {{{-0.000580783}, {-0.0151433}}, {{-0.0151433}, {-0.00125313}}}
    S_exact(0,0)=-0.000580783;S_exact(0,1)=-0.0151433;S_exact(0,2)=-0.00125313;
    //{{{0.0145037}, {-0.00235348}}, {{-0.00235348}, {-0.0639152}}}
    S_exact(1,0)=0.0145037;S_exact(1,1)=-0.00235348;S_exact(1,2)=-0.0639152;
    double eps = 1.e-6;
    bool test= true;
    for (int i=0;i<2;i++){
        for (int j=0;j<3;j++){
...
TEST(elas_fs_iso_segment,S_0_2){
    ...
    il::StaticArray2D<double,2,3> S_exact{0.};
  //
    S_exact(0,0)=0.;S_exact(0,1)=0.5;S_exact(0,2)=0.;
    S_exact(1,0)=0.;S_exact(1,1)=0.;S_exact(1,2)=0.5;
    double eps = 1.e-6;
    bool test= true;
    for (int i=0;i<2;i++){
        for (int j=0;j<3;j++){
...
TEST(elas_fs_iso_segment,W_0_1){
    ...
    //
    W_exact(0,0)=0.;W_exact(0,1)=-0.0653112;W_exact(0,2)=0.;
    W_exact(1,0)=-0.0653112;W_exact(1,1)=0.;W_exact(1,2)=-0.0653112;
...
TEST(elas_fs_iso_segment,W_0_2){
    ...
    il::StaticArray2D<double,2,3> W_exact{0.};
    //
    W_exact(0,0)=0.0077946525229993305;W_exact(0,1)=0.1401913225410614;W_exact(0,2)=-1.3328855814328973;
    W_exact(1,0)=0.1401913225410614;W_exact(1,1)=-1.3328855814328973;W_exact(1,2)=0.9364450571982305;
