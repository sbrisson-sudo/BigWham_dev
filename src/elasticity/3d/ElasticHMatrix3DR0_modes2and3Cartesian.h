...
//
// Created by Carlo Peruzzo on 18.12.22.
...
namespace bie {
    ...
    template <typename T>
    class ElasticHMatrix3DR0_modes2and3Cartesian : public bie::MatrixGenerator<T> {
     private:
     ...
     public:
      ...
      bie::ElasticProperties const elas_;
       ElasticHMatrix3DR0_modes2and3Cartesian(il::Array2D<double> &point, const il::Array<il::int_t> &permutation,
    ...
    template <typename T>
    ElasticHMatrix3DR0_modes2and3Cartesian<T>::ElasticHMatrix3DR0_modes2and3Cartesian(il::Array2D<double> &point, const il::Array<il::int_t> &permutation,
    ...
    template <typename T>
    il::int_t ElasticHMatrix3DR0_modes2and3Cartesian<T>::size(il::int_t d) const {
      IL_EXPECT_MEDIUM(d == 0 || d == 1);

      return mesh_.numberCollPts() * 2;  // num of nodes * (# of degree of freedom per node)
    }

    template <typename T>
    il::int_t ElasticHMatrix3DR0_modes2and3Cartesian<T>::blockSize() const {
      return 2;  // # of degree of freedom per node
    }

    template <typename T>
    il::int_t ElasticHMatrix3DR0_modes2and3Cartesian<T>::sizeAsBlocks(il::int_t d) const {
      IL_EXPECT_MEDIUM(d == 0 || d == 1);
    ...
    template <typename T>
    void ElasticHMatrix3DR0_modes2and3Cartesian<T>::set(il::int_t b0,
                                    il::int_t b1,
                                    il::io_t,
