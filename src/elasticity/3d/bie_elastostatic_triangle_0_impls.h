...
BieElastostatic<Triangle<0>, Triangle<0>, H>::influence(
    BoundaryElement& source_elt, il::int_t i_s, BoundaryElement& receiver_elt,
    il::int_t i_r) const {
  ...
  il::Array2D<double> el_cp_r = receiver_elt.get_collocation_points();
  ...
  il::StaticArray2D<double, 3, 3> el_vertices_s =
      source_elt.getVertices(); // StaticArray

  // get stress influence coefficients - in the local coordinate system of the
  // source element
  il::StaticArray2D<double, 3, 6> Stress =
      StressesKernelT0(receiver_coor, el_vertices_s, G, nu);

  // normal vector at the receiver location in the reference system of the
  // source element
  il::StaticArray<double, 3> nr =
      source_elt.to_local(receiver_elt.getNormal()); // il::dot(R_T_source,nr);
  ...
  il::StaticArray2D<double, 3, 3> DDs_to_traction_local{
      0.0};                                        // traction vectors
