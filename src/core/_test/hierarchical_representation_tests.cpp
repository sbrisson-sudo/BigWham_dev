...
{
    // simple mesh with close element - such that nothing can be accepted as far-field
    int n_elts=10;
    ...
    bie::HRepresentation hr= h_representation_square_matrix<bie::Segment<0>>(my_mesh,32,1.0);
...
TEST(H_representation,mesh2d_square_2)
{
    ...
    bie::Segment<0> seg0;
    bie::BEMesh<bie::Segment<0>> my_mesh(coor,conn,seg0);
    bie::HRepresentation hr= h_representation_square_matrix<bie::Segment<0>>(my_mesh,50,0.0);
...
{
    // create a mesh with 2 "clusters" of points "far apart" such that the hiearchical representation should have 2 full-block and 2 low-rank block
    int n_elts=100;
    ...
    bie::Segment<0> seg0;
    bie::BEMesh<bie::Segment<0>> my_mesh(coor,conn,seg0);
    bie::HRepresentation hr= h_representation_square_matrix<bie::Segment<0>>(my_mesh,n_elts/2,4.0);
...
{
    // create 2 mesh  one for source, one for receiver representing 2 "clusters" of points "far apart" such that the hiearchical representation should have 1 low-rank block
    int n_elts=100;
    il::Array2D<double> coor{n_elts+1,2,0.};
    il::Array2D<il::int_t> conn{n_elts,2};
    double h=0.01;
    ...
    for (int i=0;i<n_elts_r+1;i++) {
        coor_r(i,0)=i*h*2; coor_r(i,1)= 4670.;
    }
    ...
    bie::HRepresentation hr= h_representation_rectangular_matrix<bie::Segment<0>>(source_mesh,receiver_mesh,60,14.0);
