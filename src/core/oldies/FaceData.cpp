...
FaceData::FaceData(il::Array2D<double> xv, il::int_t p) {
  ...
  // inputs
  //   -xv: vertices' coordinates in the global reference system, in the
  //   following 2D array form:
  //     x0 y0 z0
  //     x1 y1 z1
  //     x2 y2 z2
  //     ...
  ...
  //     -the normal (n) and tangents (s and t) unit vectors that define the
  //     local coordinate system -the centroid (xc) coordinates -the nodes'
  //     coordinates -the collocation points' coordinates -half length of the
  //     1st edge of an element -half length of the last edge of an element

  this->vertices_ = xv;
  this->interpolation_order_ = p;
  this->NoV_ = xv.size(0);
  // check that the number of vertexes is at least 3
  if (NoV_ < 3 || xv.size(1) < 3) {
    std::cout << "FaceData - ERROR! The coordinates are non sufficient to "
                 "define a face\n";
  }
  // check that the shape of xv is (number of vertex x 3) and not (3 x number of
  // vertex)
  if (xv.size(1) > NoV_) {
    std::cout
        << "FaceData - ERROR! Please, traspose the array of vertex coordinates "
  ...
  // vec02 goes from vertex 0 to vertex NoV_ (vertex 2 in case of triangular
  // element)
  ...
  // dot product to check the collinearity of 1st, 2nd and last point of the
  // face
  //     -half length of the last edge of an element
  //     -half length of the 1st edge of an element
  double vec01norm = sqrt(il::dot(vec01, vec01)),
         vec02norm = sqrt(il::dot(vec02, vec02));
  ...
  if (dotvec12 > 0.95 && dotvec12 < 1.05) {
    std::cout
        << "FaceData - ERROR! The 2nd point of the face and the last point are "
           "collinear with the 1st point.\n Implement a way to select a "
           "different vertex to define the plane of the element\n";
  }
  ...
  this->xc_ = this->computeCentroid(
      xv); // go to function to see convention and other details
  // compute nodes' coordinates depending on the interpolation order
  this->nodes_ = this->computeNodes(); // go to function to see convention and
                                       // other details

  // compute collocation points' coordinates depending on the interpolation
  // order
  this->collocation_points_ =
      this->computeCollocationPoints(); // go to function to see convention and
                                        // other details
}
...
double FaceData::getBeta1() const { return beta1_; }

double FaceData::getBeta2() const { return beta2_; }
...
il::StaticArray<double, 3> FaceData::computeCentroid(il::Array2D<double> xv) {
  // inputs
  //   -none; it requires the previous construction of an object of the class
  //   and then uses its member variables implicitly
  // output
  //   -centroid's coordinates in the global reference system
  //
  // In the case of:
  //  - a triangular element, the centroid is the mean value of the vertex's
  //  coordinates
  //  - a quadrilateral element, the centroid is obtained from the intersection
  //  of two lines drown from 4 points.
  //    These 4 points are the centroids of all the triangles that can be
  //    generated in a quadrilateral by connecting its vertices
  ...
  default: {
    std::cout << "FaceData - ERROR! the computation of the centroid in the "
                 "case of a number of vertexes > 4 has not been implemented!\n";
  }
  }
  return xc;
}
...
il::Array2D<double> FaceData::computeNodes() {
  ...
  //     for quadratic DD elements (p = 2), 3 nodes at the vertices plus 3 nodes
  //     at the middle-edge points
  //
  //   the most general case (p = 2) has the following convention (for lower
  //   order elements the convention can be easily deducted):
  //                 0
  //               /   \
        //              /     \
        //             5       4
...
il::Array2D<double> FaceData::computeCollocationPoints() {
  ...
  //     for linear DD elements (p = 1), 3 CPs at a "distance" beta1 from each
  //     vertex (see documentation) for quadratic DD elements (p = 2), 3 CPs at
  //     a distance 'beta1' from each vertex, plus 3 CPs at a distance 'beta2'
  //     from each middle-edge node (see documentation)
  ...
  //            /   (+)   \
        ...
        //          1------3------2
  //
  //     the numbers indicate the nodes ordering, the sign + indicate the
  //     collocation points location that are ordered in the same way as the
  //     nodes, the sign (+) indicate the centroid location
  //
  //   the output is delivered in the following 2D array form (for p = 2):
  //     x0 y0 z0
  //     x1 y1 z1
  ...
  switch (this->interpolation_order_) {
  case 0: {
    if (this->NoV_ == 3) { // if 3DT0 kernel, then we "perturbate numerically'
                           // the collocation point
      // coordinates to get rid of the singularity related to the projection of
      // the triangle edges
      // TODO: compute the limits and add them in the kernel implementation

      // loop over coordinates needed because collocation_points is Array2D and
      // xc_ is Array1D
      for (il::int_t i = 0; i < 3; i++) {
        cps(0, i) = this->xc_[i] + std::numeric_limits<double>::epsilon();
        //   2.22045e-16; // perturbation by machine epsilon
      }
    } else { // else is 3DR0 or any other BE of zero interpolation order, thus
             // no perturbation
      // loop over coordinates needed because collocation_points is Array2D and
      // xc_ is Array1D
      for (il::int_t i = 0; i < 3; i++) {
  ...
  case 1: {
    il::StaticArray<double, 3>
        vec_beta; // vector that goes from vertex to centroid
  ...
  case 2: {
    il::StaticArray<double, 3>
        vec_beta1; // vector that goes from vertex to centroid
    ...
    il::StaticArray<double, 3>
        vec_beta2; // vector that goes from middle-edge node to centroid
    il::Array2D<double> nodes =
        this->computeNodes(); // because we need the middle-edge nodes
...
il::Array2D<double> FaceData::rotationMatrix(bool Transposed) {
  // inputs
  //   -Transposed; if true the matrix will rotate any vector from the global to
  //   the local coordinate system
  //                if false the matrix will rotate any vector from the local
  //                coordinate system to the global one The DEFAULT is FALSE
  // output
  //   -rotation matrix based on the local orthogonal basis (s,t,n).
  //    the rotation matrix has the following form (in the case of Transpose =
  //    true, i.e., from global to local): sx sy sz tx ty tz nx ny nz
...
il::StaticArray<double, 3> FaceData::intersectLines(
    il::StaticArray<double, 3> x1, il::StaticArray<double, 3> x2,
  ...
  if (std::abs(D42[k] * alpha - D31[k] * beta - D12[k]) > 50 * EPSILON) {
    std::cout << "FaceData - ERROR! The coordinates of the element do not lie "
                 "on a plane, "
              << std::abs(D42[k] * alpha - D31[k] * beta - D12[k]) << " is > "
              << 50 * EPSILON << " \n";
  }
