...
namespace bie {
    ...
    bie::HRepresentation
    h_representation_rectangular_matrix(bie::BEMesh<El_s> &source_mesh, bie::BEMesh<El_r> &receiver_mesh, il::int_t max_leaf_size, double eta) {
        bie::HRepresentation hr;
        hr.is_square_ = false;
        // creation of the cluster
        // first get all collocation points in the mesh
        ...
        tt.Start();
        const il::Tree<bie::SubHMatrix, 4> block_tree = bie::hmatrixTreeIxJ(Xcol_receiver, cluster_r.partition, Xcol_source, cluster_s.partition,
                                                                            eta);
        tt.Stop();
        std::cout << "Time for binary cluster tree construction  " << tt.time() << "\n";
        std::cout << " binary cluster tree depth =" << block_tree.depth() << "\n";
