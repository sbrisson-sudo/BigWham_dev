...
//// class for Triangle element
    template<int p>
    class Triangle : public Polygon<p> {
    private:
...
//   Triangle 0
    ...
    template<>
    inline void Triangle<0>::setNodes()  {
    ...
    template<>
    inline void Triangle<0>::setCollocationPoints() {
        // 0 order element: collocation at centroid
        il::Array2D<double> col{1, 3, 0.};
        for (il::int_t j = 0; j < this->spatial_dimension_; j++) {
            col(0, j) = this->centroid_[j] ;
...
// Triangle 1
    template<>
    inline Triangle<1>::Triangle() { n_nodes_=spatial_dimension_; };

    template<>
    inline void Triangle<1>::setNodes() {
    ...
    template<>
    inline void Triangle<1>::setCollocationPoints() {
...
// Triangle 2
    template<>
    inline Triangle<2>::Triangle() { n_nodes_=2*spatial_dimension_; };

    template<>
    inline void Triangle<2>::setNodes() {
        // nodes for the T2
//                             0
//                           /   \
//                          /     \
...
//                      1------3------2
    ...
    template<>
    inline void Triangle<2>::setCollocationPoints() {
        // 1 order element: collocation points
        //   col points located on the line from centroid to vertices with offset beta from vertices
