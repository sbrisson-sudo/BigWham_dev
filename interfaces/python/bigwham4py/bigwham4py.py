...
kernels_id_rec = [
    ...
    "2DS0-2DP-T",
    "2DS0-2DS0-H-mode1",
    "2DS0-2DS0-H-mode1-sym",
    "3DT0-3DT0-H",
    "3DR0-3DR0-H",
    "3DR0-3DR0-H-mode1",
    "Axi3DS0-Axi3DS0-H-mode1",
    "2DT0-2DS0-V",
    "2DR0-2DS0-V",
    "3DH0-3DR0-V",
    "3DTet0-3DT0-V",
...
class BEMatrixRectangular(LinearOperator):
    def __init__(
        ...
        if not (kernel in kernels_id_rec):
            raise Exception(
                f"Invalid kernel : {kernel}, available kernels are : [{', '.join(kernels_id_rec)}]"
            )
    ...
    def plotPattern(self, magnify_y=1.0):
        ...
        ax.add_collection(p)
        ax.set_ylim(
            [data_pattern[:, 0].min() * magnify_y, data_pattern[:, 2].max() * magnify_y]
        )
        ax.set_xlim([data_pattern[:, 1].min(), data_pattern[:, 3].max()])
