...
int testS3DP0() {
  ...
  //  std::cout << " in set h mat 2 " << cluster.permutation.size() << " e aca "
  //  << test->epsilon_aca <<"\n";
  ...
  for (il::int_t i = 0; i < pat_SPOT.size(1); i++) {
    ...
    if (h_.isFullRank(s)) {
      ...
      std::cout << "block :" << i << " | " << pat_SPOT(1, i) << ","
                << pat_SPOT(2, i) << "/ " << pat_SPOT(1, i) + A.size(0) - 1
      ...
      std::cout << "block :" << i << " | " << pat_SPOT(1, i) << ","
                << pat_SPOT(2, i) << "/ " << pat_SPOT(1, i) + A.size(0) - 1
                << "," << pat_SPOT(2, i) + B.size(0) - 1 << " - " << k << " - "
                << A.size(0) * A.size(1) + B.size(0) * B.size(1) << "\n";
    }
    std::cout << " block " << i << " :: " << pat_SPOT(0, i) << " - "
              << h_.isHierarchical(s) << "\n";
  }
  ...
  il::Array2D<double> stress = bie::computeStresses2D(
      mypts, Mesh0, elas_aux, xx, bie::point_stress_s3d_dp0_dd, 1000.);
  for (il::int_t i = 0; i < 3; i++) {
    std::cout << "stresses " << stress(i, 0) << " - " << stress(i, 1) << " - "
              << stress(i, 2) << "\n";
  }
...
int testFullMat() {
  ...
  il::Array2D<double> Mfull = serialFullMatrix2d(
      mesh, elas, bie::normal_shear_stress_kernel_dp1_dd_nodal, 0.);
  tt.Stop();
  ...
  tt.Start();
  il::Array2D<double> Mfull2 = parallelFullMatrix2d(
      mesh, elas, permut, 0., bie::normal_shear_stress_kernel_dp1_dd_nodal);
  tt.Stop();
  ...
  for (il::int_t i = 0; i < coll_points.size(0); i++) {
    std::cout << " y (1) " << i << " / " << y1[2 * i] << " y (2) " << i
              << " /  " << y2[2 * i] << "\n";
    std::cout << " y (1) " << i << " / " << y1[2 * i + 1] << " y (2) " << i
              << " / " << y2[2 * i + 1] << "\n";
  }
...
int testHdot() {
  ...
  std::cout << " Compressed memory "
            << (bie::compressionRatio(h_) * 8 * (4 * ne * 4 * ne)) << "\n";
  ...
  tt.Stop();
  std::cout << " time for getting pattern " << tt.time() << " number of blocks "
            << pattern.size(1) << "\n";
  ...
  std::cout << " time for Non-recursive hdot (parallel_invoke) " << tt.time()
            << "\n";
  std::cout << " norm y1 " << il::norm(y1, il::Norm::L2) << " y2 "
            << il::norm(y2, il::Norm::L2) << " y3 "
            << il::norm(y3, il::Norm::L2) << "\n";

  std::cout << " n FR blocks " << fr_patt.size(1) << "  n LR block "
            << lr_patt.size(1) << "\n";
...
int test3DR0() {
  ...
  // coordinates
  const std::vector<double> coor = {-1., -1., 0., 1.,  -1., 0., 1., 1., 0.,
                                    -1., 1.,  0., -1., 2.,  0., 1., 2., 0.};
  // connectivity
  const std::vector<int> conn = {0, 1, 2, 3, 3, 2, 5, 4};

  const std::vector<double> properties = {
      100, 0.2}; // Young Modulus , Poisson's ratio
  ...
  tractionHMAT.set(coor, conn, tractionKernel, properties, max_leaf_size, eta,
                   eps_aca);
  ...
  std::vector<double> coorobsp = {-10., -10., 0., 20., -20., 0.};
  std::vector<double> mysol = {1., 1., 1., 1., 1., 1.};
  std::vector<double> bb = tractionHMAT.computeStresses(
      mysol, coorobsp, 2, properties, coor, conn, true);
  ...
  std::vector<double> cc = displacementHMAT.computeDisplacements(
      mysol, coorobsp, 2, properties, coor, conn, true);
...
int test3DR0modes2and3() {
  ...
  tractionHMAT_shearONLY.set(coor, conn, tractionKernel_shearONLY, properties,
                             max_leaf_size, eta, eps_aca);
...
int test3DT0modes2and3() {
  ...
  const std::vector<double> coor = {-1., -1., 0., 1.,  -1., 0., 1., 1., 0.,
                                    -1., 1.,  0., -1., 2.,  0., 1., 2., 0.};
  // connectivity
  const std::vector<int> conn = {0, 1, 3, 1, 2, 3, 3, 2, 4, 2, 5, 4};
...
int test3DT0() {
  ...
  std::cout << " theta1, theta2, theta3: " << theta1 << " " << theta2 << " "
            << theta3 << "\n";
  ...
  std::cout << " rho_plane: " << rho_plane[0] << " " << rho_plane[1] << " "
            << rho_plane[2] << "\n";

  double eps_tol =
      2.221e-016; // 1.0e-015 parameter used for "if conditions" involving
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if ((p(i, i) + rho[i]) > eps_tol && (p(i, i1) + rho[i1]) > eps_tol) {
      ...
      double z1 = (d / p1) / p1;
      double z2 = (d / p2) / p2;
      chi[i] = log(abs(p2 / p1) * ((1.0 - 0.25 * z1 * (1.0 - 0.5 * z1)) /
  ...
  std::cout << " eta: " << eta << "\n";
  std::cout << " gamma 1: " << gamma[0] << " gamma 2: " << gamma[1]
            << " gamma 3: " << gamma[2] << "\n";
  ...
  double I7_Xi_Xi_Xi =
      bie::i7_Xi_Xi_Xi(q, d, sinAlpha, cosAlpha, D, Lambda, delta);
  double I7_Xi_Zeta_Zeta =
      bie::i7_Xi_Zeta_Zeta(q, d, sinAlpha, cosAlpha, D, Lambda, delta);
  double I7_Xi = bie::i7_Xi(d, D, delta, sinAlpha);
  double I7_Xi_Xi_Zeta =
      bie::i7_Xi_Xi_Zeta(q, d, sinAlpha, cosAlpha, D, Lambda, delta);
  double I5_Zeta = bie::i5_Zeta(delta, d, cosAlpha);
  double I7_Xi_Xi_Aux =
      bie::i7_Xi_Xi_Aux(eta, cosAlpha, Lambda, sinAlpha, q, d, D, delta);
  double I7_Xi_Xi_By_Aux =
      I7_Xi_Xi_Aux / pow(eta, 2.0) +
      theta / (15.0 * pow(eta, 3.0)); // to verify when eta =! 0 against mma
  double I5_Zeta_Zeta_Aux =
      bie::i5_Zeta_Zeta_Aux(L, sinAlpha, q, d, delta, cosAlpha);
  double I5_Zeta_Zeta_By_Aux =
      I5_Zeta_Zeta_Aux +
      theta / (3 * eta); // to verify when eta =! 0 against mma
  double I7_Xi_Zeta =
      bie::i7_Xi_Zeta(sinAlpha, Lambda, cosAlpha, q, d, D, delta);
  ...
  double I7_Zeta_Zeta_Zeta =
      bie::i7_Zeta_Zeta_Zeta(q, d, sinAlpha, cosAlpha, D, Lambda, delta);
  double I7_Zeta = bie::i7_Zeta(d, D, delta, cosAlpha);
  double I7_Zeta_Zeta_Aux =
      bie::i7_Zeta_Zeta_Aux(eta, cosAlpha, Lambda, sinAlpha, q, d, D, delta);
  double I7_Zeta_Zeta_By_Aux =
      I7_Zeta_Zeta_Aux / pow(eta, 2.0) +
      theta / (15.0 * pow(eta, 3.0)); // to verify when eta =! 0 against mma
  double I5_Xi_Xi_Aux = bie::i5_Xi_Xi_Aux(L, sinAlpha, q, d, delta, cosAlpha);
  double I5_Xi_Xi_By_Aux =
      I5_Xi_Xi_Aux + theta / (3 * eta); // to verify when eta =! 0 against mma
  ...
  double I5_By_Aux =
      I5_Aux / pow(eta, 2.0) +
      theta / (3.0 * pow(eta, 3.0)); // to verify when eta =! 0 against mma
  double I7_Aux = bie::i7_Aux(eta, q, d, D, delta);
  double I7_By_Aux =
      I7_Aux / pow(eta, 4.0) +
      theta / (5.0 * pow(eta, 5.0)); // to verify when eta =! 0 against mma
  ...
  //    double prefactor = (G/(4.0*il::pi*(1.0-nu))); // common prefactor of all
  //    coefficients
  //
  ...
  //
  //    // stress components due to the unit displacement discontinuity
  //    component DD1 (shear)
  //
  ...
  //                                               + 5.0 * I7_Xi_Zeta_Zeta ) );
  //    // s22 = b221
  //    Stress(0, 2) = prefactor * ( -3.0 * eta * ( I5_Xi - 5.0 * I7_Xi *
  //    pow(eta,2.0) ) );
  //    // s33 = b331
  //    Stress(0, 3) = prefactor * ( 3.0 * eta * ( 5.0 * I7_Xi_Xi_Zeta - I5_Zeta
  //    * nu ) );
  //    // s12 = b121
  //    Stress(0, 4) = prefactor * ( 3.0 * ( 5.0 * I7_Xi_Xi_Aux +
  //    I5_Zeta_Zeta_Aux * nu ) );
  //    // s13 = b131
  ...
  //                                               + I5_Zeta * (-1.0 + 2.0*nu) )
  //                                               ); // s11 = b112
  //    Stress(1, 1) = prefactor * ( -3.0 * eta * ( I5_Zeta - 5.0 *
  //    I7_Zeta_Zeta_Zeta ) );
  //    // s22 = b222
  //    Stress(1, 2) = prefactor * ( -3.0 * eta * ( I5_Zeta - 5.0 * I7_Zeta *
  //    pow(eta,2.0) ) );
  //    // s33 = b332
  //    Stress(1, 3) = prefactor * ( 3.0 * eta * ( 5.0 * I7_Xi_Zeta_Zeta - I5_Xi
  //    * nu ) );
  //    // s12 = b122
  //    Stress(1, 4) = prefactor * ( 15.0 * I7_Xi_Zeta * pow(eta,2.0) - 3.0 *
  //    I5_Xi_Zeta * nu );
  //    // s13 = b132
  //    Stress(1, 5) = prefactor * ( 3.0 * (5.0 * I7_Zeta_Zeta_Aux +
  //    I5_Xi_Xi_Aux * nu ) );
  //    // s23 = b232
  //
  //    // stress components due to the unit displacement discontinuity
  //    component DD3 (normal)
  //
  //    Stress(2, 0) = prefactor * ( 3.0 * ( I5_Zeta_Zeta_Aux + 5.0 *
  //    I7_Xi_Xi_Aux
  //                                         - 2.0 * I5_Zeta_Zeta_Aux * nu ) );
  //                                         // s11 = b113 TODO: it can be
  //                                         factorized
  //    Stress(2, 1) = prefactor * ( 3.0 * ( I5_Xi_Xi_Aux + 5.0 *
  //    I7_Zeta_Zeta_Aux
  //                                         - 2.0 * I5_Xi_Xi_Aux * nu ) ); //
  //                                         s22 = b223 TODO: it can be
  //                                         factorized
  //    Stress(2, 2) = prefactor * ( -6.0 * I5_Aux + 15.0 * I7_Aux );
  //    // s33 = b333
  //    Stress(2, 3) = prefactor * ( 15.0 * I7_Xi_Zeta * pow(eta,2.0)
  //                                 + I5_Xi_Zeta * (-3.0 + 6.0*nu) ); // s12 =
  //                                 b123
  //    Stress(2, 4) = prefactor * ( -3.0 * eta * ( I5_Xi - 5.0 * I7_Xi *
  //    pow(eta,2.0) ) );
  //    // s13 = b133
  //    Stress(2, 5) = prefactor * ( -3.0 * eta * ( I5_Zeta - 5.0 * I7_Zeta *
  //    pow(eta,2.0) ) );
  //    // s23 = b233
  //
  ...
  //    double prefactor = (1.0/(8.0*il::pi*(1.0-nu))); // common prefactor of
  //    all coefficients
  //
  //    il::StaticArray2D<double, 3, 3> Displacement; // output
  //
  //    // Displacement row is dof (DDx,DDy,DDx), columns are Ux,Uy,Uz in the
  //    local reference system
  //    // TODO: check if this is ok, Carlo's comment above is not what's done
  //
  //    // displacement components due to the unit displacement discontinuity
  //    DD1 (shear) Displacement(0, 0) = prefactor * ( 3.0 * I5_Xi_Xi_Aux * eta
  //    - 2.0 * theta * (-1.0 + nu) );
  //    // U1 = a11
  //    Displacement(1, 0) = prefactor * ( 3.0 * I5_Xi_Zeta * eta );
  //    // U2 = a21
  //    Displacement(2, 0) = prefactor * ( I3_Xi + 3.0 * I5_Xi * pow(eta,2.0)
  //    - 2.0 * I3_Xi * nu );
  //    // U3 = a31 TODO: it can be factorized
  //
  //    // displacement components due to the unit displacement discontinuity
  //    DD2 (shear) Displacement(0, 1) = prefactor * ( 3.0 * I5_Xi_Zeta * eta );
  //    // U1 = a12
  //    Displacement(1, 1) = prefactor * ( 3.0 * I5_Zeta_Zeta_Aux * eta - 2.0 *
  //    theta * (-1 + nu) );
  //    // U2 = a22
  ...
  //
  //    // displacement components due to the unit displacement discontinuity
  //    DD3 (normal) Displacement(0, 2) = prefactor * ( 3.0 * I5_Xi *
  //    pow(eta,2.0) + I3_Xi * (-1.0 + 2.0 * nu) );
  //    // U1 = a13
  //    Displacement(1, 2) = prefactor * ( 3.0 * I5_Zeta * pow(eta,2.0) +
  //    I3_Zeta * (-1.0 + 2.0 * nu) );
  //    // U2 = a23
  //    Displacement(2, 2) = prefactor * ( 3.0 * I5_Aux * eta - 2.0 * theta *
  //    (-1.0 + nu) );
  //    // U3 = a33
  //
...
// ///////////////////////////////////////////////////////////////////////////////
int test3DT0_PennyShaped(std::string &vertices_file,
                         std::string &connectivity_file) {

  // Make sure to use the kernel in local-local

  std::cout
      << "-------------- test3DT0 Penny-Shaped Crack ---------------------\n";
  ...
  std::cout << "Checking conversion of nodes and conn to std vectors ..."
            << "\n";
  std::cout << "coor =  " << nodes_flat[0] << " " << nodes_flat[1] << " "
            << nodes_flat[2] << "\n";
  std::cout << "conn =  " << conn_flat[0] << " " << conn_flat[1] << " "
            << conn_flat[2] << "\n";

  const std::vector<double> properties = {
      young, nu}; // Young Modulus , Poisson's ratio
  ...
  Bigwhamio test;
  test.set(nodes_flat, conn_flat, kernel_name, properties, max_leaf_size,
           eta_test, eps_aca);
  ...
  il::Array2D<double> nodes_coor =
      mesh.getNodes(); // be careful they are transposed!
  ...
  case 1: {
    ...
    for (int i = 0; i < nodes_coor.size(0); i++) {
      dd_analytical[3 * i] = ((8.0 * (lame2 + 2.0 * G) * load) /
                              (il::pi * G * (3.0 * lame2 + 4.0 * G))) *
  ...
  case 2: {
    ...
    double lame2 = (young * nu) / ((1.0 + nu) * (1.0 - 2.0 * nu));
    for (int i = 0; i < nodes_coor.size(0); i++) {
      dd_analytical[3 * i + 1] = ((8.0 * (lame2 + 2.0 * G) * load) /
                                  (il::pi * G * (3.0 * lame2 + 4.0 * G))) *
                                 sqrt(abs(pow(R, 2.0) - pow(radius[i], 2.0)));
    }
    break;
  ...
  case 3: {
    ...
    for (int i = 0; i < nodes_coor.size(0); i++) {
      dd_analytical[3 * i + 2] =
          ((8.0 * load) / (il::pi * (young / (1.0 - pow(nu, 2.0))))) *
          sqrt(abs(pow(R, 2.0) - pow(radius[i], 2.0)));
    }
  }
  }
  ...
  //            for (int i = 0; i < nodes_coor.size(0); i++){
  //                dd_analytical[3*i] = (
  //                (8.0*(lame2+2.0*G)*load)/(il::pi*G*(3.0*lame2+4.0*G)) )
  //                                     * sqrt(abs( pow(R,2.0) -
  //                                     pow(radius[i],2.0) ));
  //            }
  //            break;
  ...
  //            // For 3 (normal)
  //            for (int i = 0; i < nodes_coor.size(0); i++){
  //                dd_analytical[3*i+2] = ( (8.0*load)/( il::pi * (young/(1.0 -
  //                pow(nu,2.0))) ) )
  //                                       * sqrt(abs( pow(R,2.0) -
  //                                       pow(radius[i],2.0) ));
  //            }
  ...
  std::cout << "numerical traction ... "
            << "\n";
  for (int i = 0; i < nodes_coor.size(0); i++) {
    std::cout << radius[i] << " (" << traction_numerical[3 * i] << ", "
              << traction_numerical[3 * i + 1] << ", "
  ...
  std::vector<double> stress_tensor = test.computeStresses(
      dd_analytical_std, obsPts, 1, properties, nodes_flat, conn_flat, true);
  ...
  std::cout
      << "-------------- End of 3DT0 - Penny-shaped crack test ------------- "
      << "\n";
...
// ///////////////////////////////////////////////////////////////////////////////
int test3DT6_PennyShaped(std::string &vertices_file,
                         std::string &connectivity_file) {

  std::cout
      << "-------------- test3DT6 Penny-Shaped Crack ---------------------\n";
  ...
  //    const bie::ElasticHMatrix3DT0<double>
  //    M{coll_points,cluster.permutation,mesh,elas,1,1,1};
  //
  //    std::cout << " create h mat " << M.size(0) <<"\n";
  //    tt.Start();
  ...
  //    tt.Stop();
  //    std::cout << " create h mat ended " << h_.isBuilt() << "time" <<
  //    tt.time() << "\n"; tt.Reset(); std::cout << " compression ratio " <<
  //    il::compressionRatio(h_) << "\n";
  //
  //    // Analytical solution
  //
  //    // compute radius at all nodes ( = collocation points for T0)
  ...
  //                                  20.,-20.,0.};
  //    std::vector<double> mysol = {1.,1.,1.,1.,1.,1.};
  //    std::vector<double> bb = tractionHMAT.computeStresses(mysol, coorobsp,
  //    2, properties, coor, conn, true);
  //
  //    for(int i=0; i<bb.size()/6; ++i) {
  //        std::cout << "\n stress at point #" << i << "\n ";
  //        for(int j=0; j<6; ++j){
  ...
  std::cout
      << "-------------- End of 3DT6 - Penny-shaped crack test ------------- "
      << "\n";
...
int check3DR0() {
  ...
  std::string vertices_file = "/home/carlo/BigWhamLink/BigWhamLink/Examples/"
                              "StaticCrackBenchmarks/vertices.csv";
  std::string connectivity_file = "/home/carlo/BigWhamLink/BigWhamLink/"
                                  "Examples/StaticCrackBenchmarks/conn.csv";
  ...
  // Create and open a text file    --- TODO this is machine dependent - needs
  // to be fixed
  std::string normal_out_file = "/home/carlo/BigWhamLink/BigWhamLink/Examples/"
                                "StaticCrackBenchmarks/normalout.csv";
  std::string s1_out_file = "/home/carlo/BigWhamLink/BigWhamLink/Examples/"
                            "StaticCrackBenchmarks/s1out.csv";
  std::string s2_out_file = "/home/carlo/BigWhamLink/BigWhamLink/Examples/"
                            "StaticCrackBenchmarks/s2out.csv";
  std::string cp_out_file = "/home/carlo/BigWhamLink/BigWhamLink/Examples/"
                            "StaticCrackBenchmarks/cpout.csv";
...
#pragma omp parallel default(none) num_threads(NUMBEROFTHREADS)
  ...
  std::cout << " nodes 0 at level 2 " << listnodes[0].begin << " / "
            << listnodes[0].end << "\n";
  ...
  tt.Start();
  const il::Tree<bie::SubHMatrix, 4> block_tree2 = bie::hmatrixTreeIxJ(
      coll_points, cluster.partition, coll_points, cluster.partition, eta);
  tt.Stop();
  ...
  std::cout << " Create h mat - the old way ...  size" << M.size(0) << " * "
            << M.size(1) << "\n";
  tt.Start();
  ...
  il::Array2D<il::int_t> pattern = output_hmatPattern(h_);
  tt.Stop();
  std::cout << " time for getting pattern (old way)" << tt.time()
            << " number of blocks " << pattern.size(1) << "\n";
  tt.Reset();
  ...
  tt.Stop();
  std::cout << " time for Non-recursive hdot (parallel_invoke) "
            << tt.time() / (np + 1) << "\n";
  ...
  tt.Stop();
  std::cout << " time for Non-recursive hdot .. new way "
            << tt.time() / (np + 1) << "\n";
  std::cout << " norm y1 " << il::norm(y1, il::Norm::L2) << " y2 "
            << il::norm(y2, il::Norm::L2) << " y3 "
            << il::norm(y3, il::Norm::L2) << " y4 "
            << il::norm(y4, il::Norm::L2) << "\n";
  std::cout << "----------end of test hdot  ---------------------\n";
...
int testPl3D() {
  ...
  const bie::ElasticHMatrix3DR0_mode1Cartesian<double> M{
      coll_points, cluster.permutation, mesh, elas};
  ...
  // using bigwhamio2
  const std::vector<double> properties = {
      elas.getE(), elas.getNu()}; // Young Modulus , Poisson's ratio
...
bie:
  ...
  std::cout << "Hdot new bigwhamio " << tt.time() << " E.x norm "
            << il::norm(y, il::Norm::L2) << "\n";
  std::cout << " val " << y4[0] << "\n";
  std::vector<double> val;
  std::vector<int> pos;
  testb.getFullBlocks(val, pos);
  for (il::int_t i = 0; i < 10; i++) {
    std::cout << " val " << val[i] << "  pos size" << pos[2 * i] << "-"
              << pos[2 * i + 1] << "\n";
  }
  std::cout << " val " << val[val.size() - 1] << "  pos size"
            << pos[2 * (val.size() - 1)] << "-" << pos[2 * (val.size() - 1) + 1]
            << "\n";
  return 0;
}
/////////////////////////////////////////////////////////////////////////////////
...
int test3DT0_matrix_build() {
  ...
  // std::cout << "GiB =  " << 0.0383212 * nx * ny * nx * ny * 8. / 1024. /
  // 1024. /1024.  << " "  << "\n";
...
int main() {
  ...
  // int a = check3DR0();   // this gives an error - Can't open the file - make
  // sure that test in the main are short and can be run when pushed.
  ...
  // std::string connectivity_file =
  // "/home/alexis/bigwham/connectivity5000.csv";
  // test3DT6PennyShaped(vertices_file,connectivity_file);
  // test3DT6_PennyShaped(vertices_file,connectivity_file);

  // test3DT0();

  //// note that here you need to pass the csv files for the mesh (vertices and
  /// connectivity)
